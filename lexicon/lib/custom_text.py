...
class ConcordanceIndex(object):
    ...
    def get_concordance(self, word, width=100, lines=25):
        """
        Print a concordance for ``word`` with the specified context window.
        ...
        if offsets:
            ...
            for i in offsets:
                ...
                left1 = left
                for n in range(len(left1)):
                    if left1[0] not in ['.','?','!',';']: left1=left1[1:]
                    else: break 
                left2 = left[5:]
                for n in range(len(left2)):
                    if left2[0]!=' ': left2=left2[1:]
                    else: break
                if left1 != '': left =left1[1:]
                else: left = left2
                    
                right1 = right
                for n in range(len(right1)):
                    if right1[-1] not in ['.','?','!',';']: right1=right1[:-1]
                    else: break  
                right2 = right[:-5]
                for n in range(len(right2)):
                    if right2[-1]!= ' ': right2=right2[:-1]
                    else: break
                if right1 != '': 
                    right =right1
                else: 
                ...
                line = left+' <it>'+self._tokens[i]+'</it> '+right                
                line = ' '.join(line.split())
                lines_bundle.append(line)
                lines -= 1
        return lines_bundle
...
class Text(object):
    ...
    def concordance(self, word, width=79, lines=25):
        ...
        return self._concordance_index.get_concordance(word, lines=lines)
